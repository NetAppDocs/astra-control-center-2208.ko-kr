...
Astra Control Center를 설치하는 동안(지정된 경우 `enrolled: true` 용 `autoSupport` Astra Control Center 사용자 정의 리소스 정의(CRD) 파일 (`astra_control_center_min.yaml`), 일별 지원 번들은 에 자동으로 업로드됩니다 https://mysupport.netapp.com/site/["NetApp Support 사이트"^].
