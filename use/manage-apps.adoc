...
WARNING: 앱을 정의할 때 보호 정책이 있는 여러 앱에 Kubernetes 리소스를 포함하지 않아야 합니다. Kubernetes 리소스의 보호 정책이 중복되어 데이터 충돌이 발생할 수 있습니다. <<예: 다른 릴리즈에 대한 별도의 보호 정책,모범 사례에 대해 자세히 알아보십시오.>>
